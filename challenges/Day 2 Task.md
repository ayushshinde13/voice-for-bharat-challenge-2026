...
Yesterday your agent could talk. Today it becomes *someone*: with a job it's good at, and a list of things it will refuse to do.


For Day 2, your objective is to:

- **Step 1: Move the prompt out of the agent code.** The starter hard-codes it inside the agent class — move it into its own file and import it, so you can iterate on the prompt without touching agent code. Then **name your agent and write its backstory**: where it works, who it works for, what it knows.

- **Step 2: Define 2–3 objectives.** What does a successful call achieve? 

- **Step 3: Write your guardrails.** At minimum: what the agent must **refuse**, what it must **never claim**, and its **escalation script**.
  ...
  | Health Access | Never diagnose or name a prescription drug; escalate red-flag symptoms to a doctor or 108 |
...
- **Step 4: Add code-mixed language support.** Your agent should handle a user who starts in Hindi, drops in English words, and expects a reply in the same register or speaks in a different language entirely.

- **Step 5: Write a first-turn greeting.** 

  Structure your instructions roughly like this:
...
- **Step 6: Successfully connect to your agent** and hold a conversation showing its persona, its job and its limits.

- **Step 7: Record a short video** showing three things: the greeting, a code-mixed exchange, and a guardrail refusing an out-of-scope request.

- **Step 8: Post the video on LinkedIn** with a description of what you built on Day 2. Mention that you're building a voice agent using the fastest TTS API — **Murf Falcon**. Mention that you're part of **10 Days of Voice Agents** and don't forget to tag the official **Murf AI** handle. Also use the hashtag **#VoiceForBharat**.

- **Step 9: Submit your post link on the submission form shared on Discord**
...
- Instructions live in their own file, separate from the agent code, and are loaded at runtime
